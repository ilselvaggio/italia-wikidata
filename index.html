...
<head>
    ...
    <style>
        ...
        /* FIXED BUTTON: onclick="location.reload()" handles reset perfectly */
        ...
        #search-container { padding: 10px; border-bottom: 1px solid #34495e; display: none; }
        #catSearch { width: 90%; padding: 5px; border-radius: 4px; border: none; font-size: 12px; background: #34495e; color: white; }
        #catSearch::placeholder { color: #95a5a6; }
        #catSearch:focus { outline: none; background: #3e5871; }
        ...
        .subcat-label { flex-grow: 1; padding: 8px 5px 8px 25px; display: flex; justify-content: space-between; align-items: center;}
        .subcat-item.selected .subcat-label { padding-left: 21px; } 
...
<div id="main-container">
    <div id="sidebar">
        <div id="search-container">
            <input type="text" id="catSearch" placeholder="Cerca categoria..." oninput="filterCategories()">
        </div>
...
<script>
    ...
    var groupColors = {
        ...
        "Infrastrutture": "#7D8597", "Monumenti": "#9D4EDD", "Altro": "#000000"
    };
    ...
    function filterCategories() {
        var input = document.getElementById('catSearch').value.toLowerCase();
        var items = document.querySelectorAll('.subcat-item');
        items.forEach(item => {
            ...
            Object.keys(groupColors).forEach(group => {
                ...
                Object.keys(hierarchy[group]).sort().forEach(subgroup => {
                    ...
                    typesInSubgroup.sort((a,b) => hierarchy[group][subgroup][b] - hierarchy[group][subgroup][a]).forEach(type => {
                        ...
                        item.className = "subcat-item" + (activeCategoryFilters.has(type) ? " selected" : "");
                        
                        var labelPart = document.createElement('div');
                        labelPart.className = "subcat-label";
                        labelPart.innerHTML = `<span>${type}</span> <span>(${count})</span>`;
    ...
    function loadRegionsShape() {
        L.geoJSON(boundariesGeoJSON, {
            ...
            style: function(f) {
                ...
                return { color: color, weight: 1.5, fillOpacity: 0.6 };
            },
            onEachFeature: (f, l) => { 
                var k = getKeyFromFeature(f); 
                if (k) { 
    ...
    function changeRegion(key) {
        ...
        if(target) {
            var b = L.geoJSON(target, { pane: 'regionsPane', style: { color: "#3498db", weight: 3, fillOpacity: 0.05 } }).addTo(layers.activeBorder);
    ...
    function renderMarkers(data) {
        ...
        data.features.forEach(f => {
            ...
            var color = isD ? "#2ecc71" : (groupColors[group] || "#000000");
